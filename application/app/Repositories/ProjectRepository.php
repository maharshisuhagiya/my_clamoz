    ...
    public function search($id = '', $data = []) {
        ...
        if (in_array(request('sortorder'), array('desc', 'asc')) && request('orderby') != '') {
            ...
            if (Schema::hasColumn('projects', request('orderby'))) {
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
            ...
            case 'count_pending_tasks':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('count_pending_tasks', request('sortorder'));
                break;
            case 'count_completed_tasks':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('count_completed_tasks', request('sortorder'));
                break;
            ...
            case 'sum_all_payments':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('sum_all_payments', request('sortorder'));
                break;
            case 'sum_outstanding_balance':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('sum_outstanding_balance', request('sortorder'));
                break;
            case 'sum_hours':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('sum_hours', request('sortorder'));
                break;
            case 'sum_expenses':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('sum_expenses', request('sortorder'));
                break;
            case 'count_tickets_open':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('count_tickets_open', request('sortorder'));
                break;
            case 'count_tickets_closed':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('count_tickets_closed', request('sortorder'));
                break;
            case 'count_files':
                $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('count_files', request('sortorder'));
                break;
            }
        } else {
            //order by pinned items first and then the rest by the default sorting
            $projects->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
