    ...
    public function search($id = '', $data = []) {
        ...
        if (in_array(request('sortorder'), array('desc', 'asc')) && request('orderby') != '') {
            ...
            if (Schema::hasColumn('contracts', request('orderby'))) {
                $contracts->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
            ...
            case 'client':
                $contracts->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ...
                    ->orderBy('lead_firstname', request('sortorder'));
                break;
            case 'value':
                $contracts->orderByRaw('CASE WHEN pinned.pinned_id IS NOT NULL THEN 1 ELSE 0 END DESC')
                    ->orderBy('bill_final_amount', request('sortorder'));
                break;
            }
        } else {
            //default sorting
