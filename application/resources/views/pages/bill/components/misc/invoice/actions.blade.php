...
<div class="col-md-12  col-lg-5 align-self-center text-right p-b-9 {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        ...
        @if(auth()->user()->role->role_invoices > 1 && $bill->bill_status == 'overdue')
        <button type="button" data-toggle="tooltip" title="{{ cleanLang(__('lang.send_overdue_reminder')) }}"
            class="list-actions-button btn btn-page-actions waves-effect waves-dark confirm-action-info p-t-0"
            href="javascript:void(0)" data-confirm-title="{{ cleanLang(__('lang.send_overdue_reminder_info')) }}"
            data-confirm-text="{{ cleanLang(__('lang.confirm')) }}"
            data-url="{{ urlResource('/invoices/'.$bill->bill_invoiceid.'/overdue-reminder') }}"
